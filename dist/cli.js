...
function convertTables(text) {
  ...
  while (i2 < lines.length) {
    const line = lines[i2];
    if (line.includes("|") && i2 + 1 < lines.length && /^\|?\s*[-:]+\s*\|/.test(lines[i2 + 1])) {
      const headers = parsePipeRow(line);
      i2 += 2;
      while (i2 < lines.length && lines[i2].includes("|") && !/^\|?\s*[-:]+\s*\|/.test(lines[i2])) {
        const values = parsePipeRow(lines[i2]);
        const parts = headers.map((h2, idx) => `${h2}: ${values[idx] ?? ""}`);
